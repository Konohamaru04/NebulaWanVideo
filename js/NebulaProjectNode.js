...
async function fetchProjectFiles() {
  const res = await fetch("/nebula/projects", { credentials: "same-origin" });
  console.log("[NebulaJS] GET /nebula/projects ->", res.status);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const data = await res.json();
  console.log("[NebulaJS] list files:", data);
...
function hookAfterExec(node, handler) {
  ...
  node.onExecuted = function (output) {
    console.log("[NebulaJS] onExecuted payload:", output);
    try {
      handler(output);
    } catch (e) {
      console.warn(e);
  ...
  if (s && !node.__vp_exec_hooked) {
    ...
    s.on("executed", (msg) => {
      ...
      console.log("[NebulaJS] socket executed:", msg);
      try {
...
app.registerExtension({
  name: "NebulaProjectNode",
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "NebulaProject") return;
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      this.refreshProjectList = async (preferredFile) => {
        try {
          ...
          console.warn("[NebulaJS] refresh failed:", e);
        }
      };
      ...
      const doPartialLoad = async () => {
        ...
        try {
          const res = await fetch(`/nebula/preview?file=${encodeURIComponent(safe)}`, {
            credentials: "same-origin",
          });
          console.log("[NebulaJS] preview", safe, "->", res.status);
          if (!res.ok) throw new Error(`HTTP ${res.status}`);
          const data = await res.json();
          ...
          console.warn("[NebulaJS] preview failed:", e);
        }
      };
      ...
      const postBind = (file, pid) => {
        console.log("[NebulaJS] postBind file=", file, "pid=", pid);
        if (!file) return;
      ...
      if (!obj) { console.warn("[NebulaJS] no usable executed payload"); return; }
